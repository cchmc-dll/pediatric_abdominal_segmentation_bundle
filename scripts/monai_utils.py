...
def get_device_list(n_gpu):
    ...
    if torch.cuda.device_count() >= max(n_gpu):
        ...
        logging.info("Highest GPU ID provided in 'n_gpu' is larger than number of GPUs available, assigning GPUs starting from 0 to match n_gpu length of {}".format(len(n_gpu)))
    ...
    #     
        ...
        if distributed:
            if len(self.devices_) > 1:
                raise ValueError(f"for distributed training, `devices` must contain only 1 GPU or CPU, but got {self.devices_}.")
        ...
        super().__init__(device=self.devices_[0],
            network = self.net,
            optimizer = optimizer,
            loss_function=loss_function,
            max_epochs=max_epochs,
